...
public interface ChatGroupMapper {
    ...
    List<ChatGroupDTO> queryChatGroupPro(ChatGroupVo chatGroup);
     ...
     * @date 2024-05-20 16:19:30
     */
    List<ChatGroup> queryChatGroups();


    /**
     * 实体作为筛选条件分页查询数据
     * 特别的-->查询所有：( null,  null,  null) 等价于 {@link ChatGroupMapper#queryChatGroups()}
     *
     ...
     * @date 2024-05-20 16:19:30
     */
    List<ChatGroup> queryChatGroups(ChatGroup chatGroup, Integer offset, Integer pageSize);

    /**
     * 统计总行数<br/>
     * 与
     * {@link ChatGroupMapper#queryChatGroups(ChatGroup chatGroup, Integer offset, Integer pageSize)}配合使用，完成分页查询
     *
     ...
     * @return 影响行数
     */
    int deleteByChatGroup(ChatGroup chatGroup);
     ...
     *
     * @param entities List<ChatGroup> 实例对象列表
     * @return 影响行数
     */
    int insertBatch(@Param("chatGroups") List<ChatGroup> entities);
     ...
     */
    int insertOrUpdateBatch(@Param("chatGroups") List<ChatGroup> entities);

    List<ChatGroupDTO> queryChatGroupWithLastMessage(ChatGroupVo chatGroup);
}
