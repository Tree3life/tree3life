...
class CrudPage extends Component {
    ...
    constructor(props) {
        super(props);
        const {children, primaryKey} = this.props
        ...
        console.log('==================>>>>>>>>>>>>>>>>>>>>>>>', typeof this.state.component[2].props.children)
        // this.state.component[2].props.children
        console.log('==================>>>>>>>>>>>>>>>>>>>>>>>')
        this.columns = [...this.state.component[2].props.children]
        //配置操作列
        this.columns.push({
            title: '操作',
            render: (text, record) => {
    ...
    delete = async (id) => {
        ...
        try {
            let resp = await apiDelete({[primaryKey]: id})
    ...
    render() {
        ...
        return (
            <CRUDContext.Provider
                value={{children: this.state.children, updateTableData: this.updateTableData, ...this.props}}>
                <div>
                    <QueryForm>
...
class QueryForm extends React.Component {
    ...
    handleSearch = e => {
        const {apiQuery, primaryKey, updateTableData} = this.context
        e.preventDefault();
        this.props.form.validateFields(async (err, values) => {
            try {
                let resp = await apiQuery(values);
                updateTableData(resp.data.map(ele => {
                    return {key: ele[primaryKey], ...ele}
                }))
            } catch (error) {
                message.warn(error, 3)
